 ...
 #  hist(times, breaks=10)
  ...
  output$wordcloudT <- renderPlot({
   ...
   # made scale smaller so that it will fit all of the tweets
    ...
    #former color was Dark2 type: display.brewer.all() to see all of the color options
     wordcloud(TweetCorpus, max.words = input$wordCount, random.order = FALSE,scale=c(5,.05),colors=brewer.pal(8, "Blues"))
  })
